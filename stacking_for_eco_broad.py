...
"""
def write_images(images,fnames):
    for ii,ff in enumerate(fnames):
        hdu = fits.PrimaryHDU(images[ii])
        hdulist = fits.HDUList([hdu])
        hdulist.writeto(ff,overwrite=True)
...
if __name__=='__main__':
    ...
    if do_preparation:
        ...
        #ecocsv = ecocsv[ecocsv.g3grp_l!=825.]
        ...
        #if not images_downloaded: eco.download_images('./g3rassimages/eco_broad/')
        #eco.sort_images('./g3rassimages/eco_broad_cts/')
        #eco.measure_SNR_1Mpc('./g3rassimages/eco_broad_cts/', 5)
        eco.mask_point_sources('./g3rassimages/eco_broad_cts/', './g3rassimages/eco_broad_cts_ptsrc_rm/',\
    ...
    if scale_images:
        ...
        subobjects = [rosat_xray_stacker(tdf.g3grp_l, tdf.g3grpradeg_l, tdf.g3grpdedeg_l, tdf.g3grpcz_l, centralname=tdf.name,\
            surveys=['RASS-Int Broad']) for tdf in subframes]
        ...
        #        args=("./g3rassimages/eco_broad_exp/", "./g3rassimages/eco_broad_exp_scaled/",True,512,45,70.,2530,7470,True))
        #for jj in range(0,ncores):
        #    processes[jj].start()
        #for jj in range(0,ncores):
        #    processes[jj].join()
        ...
        for jj in range(0,ncores):
            processes[jj]=multiprocessing.Process(target=subobjects[jj].scale_subtract_images,\
                args=("./g3rassimages/eco_broad_cts_ptsrc_rm/", "./g3rassimages/eco_broad_cts_scaled/",True,512,45,70.,2530.,7470.,True))
        for jj in range(0,ncores):
            processes[jj].start()
        for jj in range(0,ncores):
            processes[jj].join()
        #eco.scale_subtract_images("./g3rassimages/eco_broad_cts/", "./g3rassimages/eco_broad_cts_scaled/", crop=True, progressConf=True, imwidth=512)
        #eco.scale_subtract_images("./g3rassimages/eco_broad_exp/", "./g3rassimages/eco_broad_exp_scaled/", crop=True, progressConf=True, imwidth=512)
        eco.to_pickle("eco_broad_prepared_images_110121.pkl")
    ...
    if stack_images:
        ...
        pickle.dump(stacks, open('ecostackedimages_broad_032722.pkl','wb'))

    if analyze:
        import matplotlib.pyplot as plt
        stackID,nbin,bincenters,counts,times = pickle.load(open('ecostackedimages_broad_032722.pkl','rb'))
        images = [ct/ex for (ct,ex) in zip(counts,times)]
        write_images(images,['stack1broad.fits','stack2broad.fits','stack3broad.fits'])
        write_images(counts,['counts1broad.fits','counts2broad.fits','counts3broad.fits'])
        ...
        apfrac = [1,1,1]# [measure_optimal_snr(images[ii],times[ii],7000,Rvir[ii])[1] for ii in range(0,len(images))]
        print(snrs)
        ...
        for jj in range(0,len(bincenters)):
            ...
            circ=plt.Circle((visimages[jj].shape[0]//2, visimages[jj].shape[1]/2), apfrac[jj]*(0.5*Rvir[jj]/(100))*206265/45,\
                edgecolor='red', facecolor='None', label=str(apfrac[jj])+r'$R_{\rm vir}$')
            axs[0][jj].add_patch(circ)
